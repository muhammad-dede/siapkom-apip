...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-3">Tambah Diklat</h4>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        <form action="{{ route('diklat.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                                <label for="id_jenis_diklat" class="form-label">Jenis</label>
                                <select class="form-select @error('id_jenis_diklat') is-invalid @enderror"
                                    ...
                                    @foreach (refJenisDiklat() as $row)
                                        <option value="{{ $row->id_jenis_diklat }}"
                                            {{ old('id_jenis_diklat') == $row->id_jenis_diklat ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('nama_diklat') is-invalid @enderror"
                                    id="nama_diklat" value="{{ old('nama_diklat') }}" name="nama_diklat" />
                                @error('nama_diklat')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
