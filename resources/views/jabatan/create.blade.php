...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-3">Tambah Jabatan</h4>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        <form action="{{ route('jabatan.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <select class="form-select @error('id_jenis_jabatan') is-invalid @enderror"
                                    ...
                                    @foreach (refJenisJabatan() as $row)
                                        <option value="{{ $row->id_jenis_jabatan }}"
                                            {{ old('id_jenis_jabatan') == $row->id_jenis_jabatan ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('nama_jabatan') is-invalid @enderror"
                                    id="nama_jabatan" value="{{ old('nama_jabatan') }}" name="nama_jabatan" />
                                @error('nama_jabatan')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
