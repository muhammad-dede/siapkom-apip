...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-3">Edit User</h4>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        <form action="{{ route('user.update', $user->id_user) }}" method="POST"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('nama') is-invalid @enderror"
                                    id="nama" placeholder="John Doe" value="{{ old('nama') ?? $user->nama }}"
                                    name="nama" />
                                @error('nama')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('username') is-invalid @enderror"
                                    id="username" placeholder="johndoe" value="{{ old('username') ?? $user->username }}"
                                    name="username" />
                                @error('username')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                ...
                                <select class="form-select @error('id_role') is-invalid @enderror" id="id_role"
                                    ...
                                    @foreach (getRole() as $row)
                                        <option value="{{ $row->id_role }}"
                                            {{ old('id_role') ?? $user->id_role == $row->id_role ? 'selected' : '' }}>
