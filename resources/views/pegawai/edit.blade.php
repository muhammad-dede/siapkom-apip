...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-3">Edit Pegawai</h4>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        <form action="{{ route('pegawai.update', $pegawai->id_pegawai) }}" method="POST"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('nama_pegawai') is-invalid @enderror"
                                    id="nama_pegawai" value="{{ old('nama_pegawai') ?? $pegawai->nama_pegawai }}"
                                    name="nama_pegawai" />
                                @error('nama_pegawai')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('nip') is-invalid @enderror" id="nip"
                                    value="{{ old('nip') ?? $pegawai->nip }}" name="nip" />
                                @error('nip')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                    ...
                                    <select class="form-select @error('id_pangkat') is-invalid @enderror" id="id_pangkat"
                                        ...
                                        @foreach (refPangkat() as $row)
                                            <option value="{{ $row->id_pangkat }}"
                                                {{ old('id_pangkat') ?? $pegawai->id_pangkat == $row->id_pangkat ? 'selected' : '' }}>
                                    ...
                                    <select class="form-select @error('id_golongan') is-invalid @enderror" id="id_golongan"
                                        ...
                                        @foreach (refGolongan() as $row)
                                            <option value="{{ $row->id_golongan }}"
                                                {{ old('id_golongan') ?? $pegawai->id_golongan == $row->id_golongan ? 'selected' : '' }}>
                                ...
                                <select class="form-select @error('id_jabatan') is-invalid @enderror" id="id_jabatan"
                                    ...
                                    @foreach (refJabatan() as $row)
                                        <option value="{{ $row->id_jabatan }}"
                                            {{ old('id_jabatan') ?? $pegawai->id_jabatan == $row->id_jabatan ? 'selected' : '' }}>
